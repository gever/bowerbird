...
def handle_pilotview(noun):
    ...
    nav = render_nav_header()
    pg = render_template('std_page', {'refresh':0, 'content':pilot_info, 'nav':nav, 'adminnav':'', 'preamble':'', 'last_reset':LastResetTime.strftime(LastResetFormat)})
    return pg
...
def handle_pilotadmin(noun):
    ...
    nav = render_nav_header()
    adminnav = render_nav_admin_header()
    pg = render_template('std_page', {'refresh':0, 'content':pilot_info, 'nav':nav, 'adminnav':adminnav, 'preamble':'', 'last_reset':LastResetTime.strftime(LastResetFormat)})
    return pg
